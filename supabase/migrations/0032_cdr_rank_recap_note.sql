-- Ajoute la note CDR au contrat de la RPC historique.
-- PostgreSQL ne peut pas modifier les colonnes RETURNS TABLE avec CREATE OR
-- REPLACE : le DROP sans CASCADE reste transactionnel et échoue sans effet si
-- une dépendance SQL non prévue existe.
drop function public.get_cdr_rank_recap(uuid);

create function public.get_cdr_rank_recap(
