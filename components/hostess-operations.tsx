...
export function HostessOperations({ view }: { view: OperationView }) {
  ...
  async function refresh() {
    ...
    if (!currentNight) {
      setNotes([]); setPromoters([]); setPromoterEvents([]); setAuditRows([]); setActorProfiles([]); setEntries([]); setSelectedPromoterId(null); setNotice(''); setLoading(false);
      return;
    }
    const [notesResult, promotersResult, promoterEventsResult, entriesResult, auditResult] = await Promise.all([
      ...
      supabase.from('operational_audit_log').select('*').eq('night_session_id', currentNight).order('created_at', { ascending: false }),
    ]);
    if (notesResult.error || promotersResult.error || promoterEventsResult.error || entriesResult.error || auditResult.error) {
      console.error('[OPERATIONS] Chargement impossible.', { notesError: notesResult.error, promotersError: promotersResult.error, promoterEventsError: promoterEventsResult.error, entriesError: entriesResult.error, auditError: auditResult.error });
      setNotice(`Impossible de charger les données : ${notesResult.error?.message ?? promotersResult.error?.message ?? entriesResult.error?.message ?? promoterEventsResult.error?.message ?? auditResult.error?.message}`);
      setLoading(false);
      return;
    }
    ...
    const audits = (auditResult.data ?? []) as OperationalAuditLog[];
    setAuditRows(audits);
    const actorIds = actorIdsForResolution(
      ...audits.map((audit) => audit.actor_id),
      ...(notesResult.data ?? []).map((note: FloorNote) => note.created_by),
      ...(promotersResult.data ?? []).map((promoter: Promoter) => promoter.created_by),
      ...(promoterEventsResult.data ?? []).map((event: PromoterCountEvent) => event.changed_by),
      ...(entriesResult.data ?? []).map((entry: ClubEntryCount) => entry.created_by),
    );
    ...
    if (profilesError) console.error('[OPERATIONS] Résolution des auteurs impossible.', profilesError);
    setActorProfiles((profiles ?? []) as OperationalActorProfile[]);
    console.info('[OPERATIONS] Listes Supabase chargées.', { nightSessionId: currentNight, notes: notesResult.data?.length ?? 0, promoters: promotersResult.data?.length ?? 0, promoterEvents: promoterEventsResult.data?.length ?? 0, entryCounts: entriesResult.data?.length ?? 0, audits: audits.length });
    setLoading(false);
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('hostess-operations')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'operational_audit_log' }, () => void refresh())
      .subscribe();
  ...
  if (view === 'piste') return <section><header className="mb-5"><p className="text-xs uppercase tracking-[.2em] text-fuchsia-400 sm:text-sm sm:tracking-[.25em]">Opérations</p><h1 className="text-2xl font-black sm:text-3xl">PISTE</h1></header><div className="panel p-4"><textarea aria-label="Ajouter une note Piste" value={noteText} onChange={(event) => setNoteText(event.target.value)} placeholder="Ajouter une note Piste..." className="min-h-24 w-full rounded-xl bg-zinc-800 p-3 outline-none ring-1 ring-zinc-700 focus:ring-violet-400" /><button disabled={!noteText.trim()} onClick={() => void addNote()} className="mt-3 min-h-12 w-full rounded-xl bg-fuchsia-600 px-5 py-3 font-bold disabled:opacity-40 sm:w-auto">Ajouter</button></div><h2 className="mt-5 font-bold">HISTORIQUE PISTE</h2><div className="mt-3 grid gap-3">{notes.length > 0 ? notes.map((note) => <article className="panel p-4" key={note.id}>{editingNote === note.id ? <NoteEditor note={note} onCancel={() => setEditingNote(null)} onSave={saveNote} /> : <><div className="flex flex-wrap gap-x-3 gap-y-1"><p className="mr-auto text-sm text-zinc-400">{dateTime(note.created_at)}</p><button onClick={() => setEditingNote(note.id)} className="min-h-11 text-sm text-violet-300">Modifier</button><button onClick={() => void deleteNote(note.id)} className="min-h-11 text-sm text-red-300">Supprimer</button></div><p className="mt-2 break-words">{note.content}</p><p className="mt-3 break-words text-xs text-zinc-500">Créé par {formatActorLabel(actors.get(note.created_by ?? auditActor('floor_note', note.id, ['floor_note.created']) ?? ''))}</p>{auditActor('floor_note', note.id, ['floor_note.updated']) && <p className="mt-1 break-words text-xs text-zinc-500">Modifié par {formatActorLabel(actors.get(auditActor('floor_note', note.id, ['floor_note.updated'])!))}</p>}</>}</article>) : loading ? <p className="text-sm text-zinc-400">Chargement…</p> : <p className="text-sm text-zinc-400">Aucune note pour la soirée active.</p>}</div>{notice && <p className="mt-4 break-words text-sm text-red-300">{notice}</p>}</section>;

  if (view === 'promoteurs') return selectedPromoter ? <PromoterDetail summary={selectedPromoter} actors={actors} auditActor={auditActor} saving={savingActivity} onBack={() => setSelectedPromoterId(null)} onAdd={addPromoterActivity} onUpdate={updatePromoterActivity} onDeleteActivity={deletePromoterActivity} onDeletePromoter={deletePromoter} notice={notice} /> : <PromoterOverview summaries={promoterSummaries} actors={actors} auditActor={auditActor} total={promoterTotal} arrivalCount={promoterArrivals} loading={loading} promoterName={promoterName} onPromoterNameChange={setPromoterName} onAddPromoter={addPromoter} onOpen={setSelectedPromoterId} notice={notice} />;

  return <section><header className="mb-5"><p className="text-xs uppercase tracking-[.2em] text-fuchsia-400 sm:text-sm sm:tracking-[.25em]">Opérations</p><h1 className="text-2xl font-black sm:text-3xl">ENTRÉES CLUB</h1><p className="mt-2 text-zinc-400">TOTAL ENTRÉES CLUB · <b className="text-white">{clubEntryTotal} personnes</b></p>{latestEntry && <p className="mt-1 text-sm text-zinc-400">Dernier relevé : {dateTime(latestEntry.recorded_at)} — {latestEntry.count} personnes</p>}{reminder && <p className="mt-2 text-sm text-violet-200">Un nouveau relevé peut être effectué.</p>}</header><div className="panel p-4"><label className="text-sm font-semibold">Entrées club<input aria-label="Entrées club" inputMode="numeric" type="number" min="0" value={entryValue} onChange={(event) => setEntryValue(event.target.value)} className="mt-2 block min-h-12 w-full rounded-xl bg-zinc-800 p-3 text-xl font-bold outline-none ring-1 ring-zinc-700 focus:ring-violet-400" /></label><button onClick={() => void saveEntry()} className="mt-3 min-h-12 w-full rounded-xl bg-fuchsia-600 px-5 py-3 font-bold sm:w-auto">Enregistrer le relevé</button></div><div className="mt-4"><h2 className="font-bold">Derniers relevés</h2><div className="mt-3 grid gap-2">{entries.length > 0 ? entries.map((entry) => editingEntry === entry.id ? <EntryEditor entry={entry} onCancel={() => { setEditingEntry(null); setEntryValue(''); }} onSave={(item, count) => saveEntry(item, count)} key={entry.id} /> : <article className="panel flex flex-wrap items-center gap-3 p-3" key={entry.id}><span className="text-zinc-400">{dateTime(entry.recorded_at)}</span><span className="mr-auto min-w-0"><b className="block text-xl">{entry.count} personnes</b><small className="break-words text-zinc-500">Par {formatActorLabel(actors.get(entry.created_by ?? ''))}</small></span><button onClick={() => { setEditingEntry(entry.id); setEntryValue(String(entry.count)); }} className="min-h-11 text-sm text-violet-300">Modifier</button><button onClick={() => void deleteEntry(entry.id)} className="min-h-11 text-sm text-red-300">Supprimer</button></article>) : loading ? <p className="text-sm text-zinc-400">Chargement…</p> : <p className="text-sm text-zinc-400">Aucun relevé pour la soirée active.</p>}</div></div>{notice && <p className="mt-4 break-words text-sm text-red-300">{notice}</p>}</section>;
}

function PromoterOverview({ summaries, actors, auditActor, total, arrivalCount, loading, promoterName, onPromoterNameChange, onAddPromoter, onOpen, notice }: { summaries: ReturnType<typeof promoterActivitySummaries>; actors: Map<string, OperationalActorProfile>; auditActor: (entityType: string, entityId: string, actionTypes?: string[]) => string | null; total: number; arrivalCount: number; loading: boolean; promoterName: string; onPromoterNameChange: (value: string) => void; onAddPromoter: () => Promise<void>; onOpen: (id: string) => void; notice: string }) {
  return <section className="min-w-0"><header className="mb-4 sm:mb-5"><p className="text-xs uppercase tracking-[.2em] text-fuchsia-400 sm:text-sm sm:tracking-[.25em]">Opérations</p><h1 className="text-2xl font-black sm:text-3xl">PROMOTEURS</h1><div className="mt-3 grid grid-cols-3 gap-2 text-center"><div className="panel p-3"><p className="text-xs text-zinc-400">TOTAL APPORTÉ</p><b>{total}</b><p className="text-xs text-zinc-400">personnes</p></div><div className="panel p-3"><p className="text-xs text-zinc-400">PROMOTEURS</p><b>{summaries.length}</b></div><div className="panel p-3"><p className="text-xs text-zinc-400">ARRIVÉES</p><b>{arrivalCount}</b></div></div></header><div className="panel flex flex-col gap-3 p-3 sm:flex-row sm:p-4"><input aria-label="Nom du promoteur" value={promoterName} onChange={(event) => onPromoterNameChange(event.target.value)} placeholder="Nom du promoteur" className="min-h-12 min-w-0 flex-1 rounded-xl bg-zinc-800 px-3 outline-none ring-1 ring-zinc-700 focus:ring-violet-400" /><button disabled={!promoterName.trim()} onClick={() => void onAddPromoter()} className="min-h-12 rounded-xl bg-fuchsia-600 px-4 py-3 font-bold disabled:opacity-40">Ajouter</button></div><h2 className="mt-5 font-bold">PROMOTEURS DE LA SOIRÉE</h2><div className="mt-3 grid gap-3 sm:grid-cols-2">{summaries.map(({ promoter, activities, lastActivity }) => <button type="button" key={promoter.id} onClick={() => onOpen(promoter.id)} className="panel w-full p-3 text-left sm:p-4 transition hover:ring-1 hover:ring-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-400"><div className="flex items-center gap-3"><b className="mr-auto text-xl">{promoter.name}</b><span aria-hidden className="text-violet-300">›</span></div><p className="mt-2 text-zinc-300">{promoter.entry_count} personnes · {activities.length} arrivée{activities.length > 1 ? 's' : ''}</p>{lastActivity ? <p className="mt-3 text-sm text-zinc-400">Dernière arrivée<br />{dateTime(lastActivity.created_at)} · +{lastActivity.people_added} personnes</p> : <p className="mt-3 text-sm text-zinc-500">Aucune arrivée enregistrée</p>}</button>)}</div>{!loading && summaries.length === 0 && <p className="mt-4 text-sm text-zinc-400">Aucun promoteur pour la soirée active.</p>}{loading && <p className="mt-4 text-sm text-zinc-400">Chargement…</p>}{notice && <p className="mt-4 text-sm text-red-300">{notice}</p>}</section>;
}

function PromoterDetail({ summary, actors, auditActor, saving, onBack, onAdd, onUpdate, onDeleteActivity, onDeletePromoter, notice }: { summary: ReturnType<typeof promoterActivitySummaries>[number]; actors: Map<string, OperationalActorProfile>; auditActor: (entityType: string, entityId: string, actionTypes?: string[]) => string | null; saving: boolean; onBack: () => void; onAdd: (promoter: Promoter, peopleAdded: number, note: string) => Promise<boolean>; onUpdate: (event: PromoterCountEvent, peopleAdded: number, note: string) => Promise<boolean>; onDeleteActivity: (event: PromoterCountEvent) => Promise<void>; onDeletePromoter: (id: string) => Promise<void>; notice: string }) {
...
function PromoterActivityEditor({ event, saving, onCancel, onSave }: { event: PromoterCountEvent; saving: boolean; onCancel: () => void; onSave: (people: number, note: string) => Promise<boolean> }) {
  ...
  return <article className="panel p-4"><p className="text-sm text-zinc-400">{dateTime(event.created_at)}</p><label className="mt-2 block text-sm font-semibold">Nombre de personnes<input aria-label="Modifier arrivée promoteur" inputMode="numeric" pattern="[0-9]*" value={people} onChange={(item) => setPeople(normalizePositiveInput(item.target.value))} className="mt-2 block w-full rounded-lg bg-zinc-800 p-2 text-xl font-bold" /></label><label className="mt-3 block text-sm font-semibold">Note<textarea aria-label="Modifier note arrivée promoteur" value={note} maxLength={500} onChange={(item) => setNote(item.target.value)} className="mt-2 block min-h-20 w-full rounded-lg bg-zinc-800 p-2" /></label><div className="mt-3 grid grid-cols-2 gap-2"><button disabled={!validPeople || saving} onClick={() => validPeople && void onSave(validPeople, note)} className="min-h-11 rounded-lg bg-fuchsia-600 px-3 py-2 text-sm font-bold disabled:opacity-40">Enregistrer</button><button disabled={saving} onClick={onCancel} className="min-h-11 rounded-lg bg-zinc-800 px-3 py-2 text-sm font-bold">Annuler</button></div></article>;
}

function NoteEditor({ note, onCancel, onSave }: { note: FloorNote; onCancel: () => void; onSave: (note: FloorNote, content: string) => Promise<void> }) { const [content, setContent] = useState(note.content); return <><textarea aria-label="Modifier note Piste" value={content} onChange={(event) => setContent(event.target.value)} className="min-h-20 w-full rounded-xl bg-zinc-800 p-3" /><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => void onSave(note, content)} className="min-h-11 rounded-lg bg-fuchsia-600 px-3 py-2 text-sm font-bold">Enregistrer</button><button onClick={onCancel} className="min-h-11 rounded-lg bg-zinc-800 px-3 py-2 text-sm font-bold">Annuler</button></div></>; }
function EntryEditor({ entry, onCancel, onSave }: { entry: ClubEntryCount; onCancel: () => void; onSave: (entry: ClubEntryCount, count: number) => Promise<void> }) { const [input, setInput] = useState(String(entry.count)); return <article className="panel p-3"><p className="text-sm text-zinc-400">{dateTime(entry.recorded_at)}</p><input aria-label="Corriger relevé" type="number" min="0" inputMode="numeric" value={input} onFocus={() => { if (nonNegative(Number(input)) === 0) setInput(''); }} onChange={(event) => setInput(event.target.value)} onBlur={() => { if (input === '') setInput('0'); }} className="mt-2 w-full rounded-lg bg-zinc-800 p-2 text-xl font-bold" /><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => void onSave(entry, nonNegative(Number(input)))} className="min-h-11 rounded-lg bg-fuchsia-600 px-3 py-2 text-sm font-bold">Enregistrer</button><button onClick={onCancel} className="min-h-11 rounded-lg bg-zinc-800 px-3 py-2 text-sm font-bold">Annuler</button></div></article>; }
