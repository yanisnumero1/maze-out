...
function PromoterCard({ promoter, onSave, onDelete }: { promoter: Promoter; onSave: (promoter: Promoter, value: number) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const [input, setInput] = useState(String(promoter.entry_count));
  useEffect(() => setInput(String(promoter.entry_count)), [promoter.entry_count]);
  const current = nonNegative(Number(input));
  const commit = (next: number) => { const safe = nonNegative(next); setInput(String(safe)); void onSave(promoter, safe); };
  return <article className="panel p-4"><div className="flex gap-3"><b className="mr-auto text-xl">{promoter.name}</b><button onClick={() => void onDelete(promoter.id)} className="text-sm text-red-300">Supprimer</button></div><div className="mt-4 flex items-center justify-between gap-3"><button onClick={() => commit(current - 1)} disabled={current === 0} className="h-11 w-11 rounded-lg bg-zinc-800 text-2xl disabled:opacity-40">−</button><input aria-label={'Compteur ' + promoter.name} type="number" min="0" inputMode="numeric" value={input} onFocus={() => { if (current === 0) setInput(''); }} onChange={(event) => setInput(event.target.value)} onBlur={() => commit(input === '' ? 0 : Number(input))} className="h-11 w-24 rounded-lg bg-zinc-800 text-center text-xl font-bold outline-none ring-1 ring-zinc-700" /><button onClick={() => commit(current + 1)} className="h-11 w-11 rounded-lg bg-fuchsia-600 text-2xl">+</button></div><p className="mt-3 text-sm text-zinc-400">{current} personnes</p></article>;
}
...
function EntryEditor({ entry, onCancel, onSave }: { entry: ClubEntryCount; onCancel: () => void; onSave: (entry: ClubEntryCount, count: number) => Promise<void> }) {
  const [input, setInput] = useState(String(entry.count));
