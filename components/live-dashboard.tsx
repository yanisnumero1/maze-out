...
import type { ArrivalDraft, BusinessReferrer, LiveTable, OperationalActorProfile, OperationalAuditLog, TableVisit, TableVisitTransfer, Zone } from '@/lib/types';
...
export function LiveDashboard({ initialTables }: { initialTables: LiveTable[] }) {
  ...
  useEffect(() => {
    ...
    async function loadNightData(nightId: string) {
      ...
      if (!startedAt) {
        currentNightId = null;
        if (active) { setActiveNightId(null); setNightStartedAt(null); setDrafts([]); setVisits([]); setTransfers([]); setAuditRows([]); setActorProfiles([]); setBusinessReferrers([]); }
        return;
      }
  ...
  return <>
    <header className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-fuchsia-400">LIVE</p><h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">MAZE-OUT</h1><p className="mt-1 text-sm text-zinc-400">{activeNightId ? nightStartedAt && now ? `Soirée en cours · ${formatNightElapsed(nightStartedAt, now)}` : 'Soirée en cours' : 'Aucune soirée active'}</p></div><div className="flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs font-bold"><span aria-hidden="true" className={`h-2 w-2 rounded-full ${connection === 'live' ? 'bg-emerald-400' : connection === 'offline' ? 'bg-red-400' : 'bg-orange-400'}`} /><span className={connectionColors[connection]}>{connectionLabels[connection]}</span></div></header>
    {loading ? <div className="panel p-6 text-zinc-300">Chargement…</div> : error ? <div className="panel border-red-500/40 p-6 text-red-200"><p>Impossible de charger les données</p><p className="mt-1 text-sm text-red-200/70">{error}</p></div> : zones.length === 0 ? <div className="panel p-6 text-zinc-300">Aucune donnée disponible</div> : <>
      ...
      <section className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 [&>article:last-child]:col-span-2 sm:[&>article:last-child]:col-span-1" aria-label="Indicateurs Live"><article className="rounded-xl bg-zinc-900/70 p-3 sm:rounded-2xl sm:p-4"><p className="text-[11px] font-bold uppercase tracking-wide text-zinc-500">Tables</p><p className="mt-1 text-2xl font-black sm:text-3xl">{dashboard.occupied}<span className="text-sm text-zinc-600">/{tables.length}</span></p><p className="mt-1 text-xs text-zinc-400">occupées</p></article><article className="rounded-xl bg-zinc-900/70 p-3 sm:rounded-2xl sm:p-4"><p className="text-[11px] font-bold uppercase tracking-wide text-zinc-500">Personnes</p><p className="mt-1 text-2xl font-black sm:text-3xl">{dashboard.present}</p><p className="mt-1 text-xs text-zinc-400">présentes</p></article><article className="rounded-xl bg-zinc-900/70 p-3 sm:rounded-2xl sm:p-4"><p className="text-[11px] font-bold uppercase tracking-wide text-zinc-500">Attentes</p><p className="mt-1 text-2xl font-black text-orange-200 sm:text-3xl">{dashboard.activeDraftCount}</p><p className="mt-1 text-xs text-zinc-400">{dashboard.pendingPeople} personnes</p></article></section>
