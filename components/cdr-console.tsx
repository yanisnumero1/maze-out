...
export function CdrConsole() {
  ...
  return <><main className="cdr-screen mx-auto min-h-dvh max-w-2xl px-4 py-4 sm:px-6">
    ...
    <section className="cdr-print-sales" aria-label="Ventes du rang">{selectedRecapSales.length ? selectedRecapSales.map((sale) => {
      ...
      return <article className="cdr-print-sale" key={sale.table_visit_id}><div className="cdr-print-sale-title"><h2>Vente #{sale.sale_number ?? '—'}</h2><p>{transferred ? `Table ${sale.source_table_number} → Table ${sale.final_table_number}` : `Table ${sale.final_table_number || sale.source_table_number || '—'}`}</p></div><dl><div><dt>Réservation</dt><dd>{sale.reservation_name || 'Non renseignée'}</dd></div><div><dt>Conso</dt><dd>{sale.consumption || 'Non renseignée'}</dd></div><div><dt>Commentaire de vente</dt><dd>{sale.sale_comment || '—'}</dd></div>{sale.cdr_comment && <div><dt>Note CDR historique</dt><dd>{sale.cdr_comment}</dd></div>}<div><dt>Montant</dt><dd>{sale.cdr_amount === null ? 'Non renseigné' : formatCdrAmount(Number(sale.cdr_amount))}</dd></div><div><dt>{sale.business_referrer_name ? 'Apporteur validé' : 'Apporteur proposé'}</dt><dd>{referrer}</dd></div></dl></article>;
