      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2" aria-label="Mes tables">
        {orderedTables.map((table) => {
          ...
          return <article className={`min-h-28 rounded-2xl border p-4 ${occupied ? 'border-fuchsia-500/50 bg-fuchsia-950/30' : 'border-zinc-700 bg-zinc-900/80'}`} key={table.id}>
            <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-black">Table {table.display_number ?? table.number}</h2><p className="mt-1 text-sm text-zinc-400">{table.zone?.name ?? 'Carré non attribué'}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${occupied ? 'bg-fuchsia-500/20 text-fuchsia-200' : 'bg-zinc-700 text-zinc-300'}`}>{occupied ? 'OCCUPÉE' : 'LIBRE'}</span></div>
