        ...
        {tableRows.map(({ table, visit }) => {
          ...
          return <article className={`min-h-28 rounded-2xl border p-4 ${occupied ? 'border-fuchsia-500/50 bg-fuchsia-950/30' : 'border-zinc-700 bg-zinc-900/80'}`} key={table.id}>
            ...
            {visit && noteDraft && <div className="mt-5 grid gap-4 border-t border-white/10 pt-4">
              ...
              <section className="rounded-xl bg-zinc-950/60 p-3"><p className="text-sm font-semibold">Apporteur validé</p>{hasValidatedReferrer && <p className="mt-1 text-sm text-emerald-300">Validé : {validatedName ?? 'Apporteur validé'}</p>}<label className="mt-3 block text-sm text-zinc-300"><span className="sr-only">Apporteur validé</span><input disabled={rankIsReadOnly} list="cdr-business-referrer-options" maxLength={300} value={validatedReferrers[visit.id] ?? ''} onChange={(event) => { setValidatedReferrers((current) => ({ ...current, [visit.id]: event.target.value })); setValidationStates((current) => ({ ...current, [visit.id]: 'idle' })); }} placeholder={visit.proposed_business_referrer_name || 'Choisir ou saisir un apporteur'} className="min-h-11 w-full rounded-xl border border-zinc-700 bg-zinc-900 p-3 outline-none focus:border-fuchsia-400 disabled:cursor-not-allowed disabled:opacity-60" /></label><button disabled={rankIsReadOnly || validationState === 'saving'} onClick={() => void validateBusinessReferrer(visit)} className="mt-3 min-h-11 rounded-xl bg-fuchsia-600 px-4 py-2 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-60">{validationState === 'saving' ? 'Validation...' : hasValidatedReferrer ? 'Corriger l’apporteur' : 'Valider l’apporteur'}</button>{validationMessages[visit.id] && <p className={validationState === 'error' ? 'mt-2 text-sm text-red-300' : 'mt-2 text-sm text-emerald-300'}>{validationMessages[visit.id]}</p>}</section>
