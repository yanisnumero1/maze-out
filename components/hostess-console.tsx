...
export function HostessConsole({ tables: initialTables }: { tables: LiveTable[] }) {
  ...
  if (screen === 'waiters') return <>
    ...
    <GlobalSearch tables={tables} drafts={drafts} visits={tableVisits} businessReferrers={businessReferrers} promoters={promoters} onSelectTable={(table) => router.replace(`/hostess?table=${encodeURIComponent(getTableDisplayNumber(table))}`)} onSelectPromoter={(promoter) => router.replace(`/hostess?view=promoteurs&promoter=${encodeURIComponent(promoter.id)}`)} onSelectCdr={openCdr} />
    <section aria-label={`CDR de ${zone?.name ?? 'ce carré'}`} className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{cdrRanks.map((rank) => <button key={rank.headWaiter.id} onClick={() => openRank(rank.headWaiter.id)} className="panel min-h-36 min-w-0 p-4 text-left transition hover:border-violet-400/60 hover:bg-zinc-900 sm:p-5"><b className="block break-words text-xl font-black">{rank.headWaiter.first_name} {rank.headWaiter.last_name}</b><span className="mt-4 block text-sm font-semibold text-zinc-200">{rank.occupiedTables} table{rank.occupiedTables !== 1 ? 's' : ''} occupée{rank.occupiedTables !== 1 ? 's' : ''}</span><span className="mt-1 block text-2xl font-black text-fuchsia-200">{rank.presentPeople} <span className="text-sm font-semibold text-zinc-400">personne{rank.presentPeople !== 1 ? 's' : ''}</span></span></button>)}</section>
