...
function RankTableIndicator({ table }: { table: LiveTable }) {
  const occupied = presentTotal(table) > 0;
  const state = occupied ? 'occupée' : 'libre';
  return <span aria-label={`Table ${getTableDisplayNumber(table)} — ${state}`} title={`Table ${getTableDisplayNumber(table)} — ${state}`} className={occupied ? 'flex h-11 min-w-11 items-center justify-center rounded-lg border border-red-400/40 bg-red-500/15 px-2 font-mono text-sm font-black text-red-100' : 'flex h-11 min-w-11 items-center justify-center rounded-lg border border-emerald-400/40 bg-emerald-500/15 px-2 font-mono text-sm font-black text-emerald-100'}>{getTableDisplayNumber(table)}</span>;
}
...
export function HostessConsole({ tables: initialTables }: { tables: LiveTable[] }) {
  ...
  if (screen === 'waiters') return <>
    ...
    <GlobalSearch tables={tables} drafts={drafts} visits={tableVisits} businessReferrers={businessReferrers} promoters={promoters} onSelectTable={(table) => router.replace(`/hostess?table=${encodeURIComponent(getTableDisplayNumber(table))}`)} onSelectPromoter={(promoter) => router.replace(`/hostess?view=promoteurs&promoter=${encodeURIComponent(promoter.id)}`)} onSelectCdr={openCdr} />
    <section aria-label={`CDR de ${zone?.name ?? 'ce carré'}`} className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{cdrRanks.map((rank) => <button key={rank.headWaiter.id} onClick={() => openRank(rank.headWaiter.id)} className="panel min-w-0 p-4 text-left transition hover:border-violet-400/60 hover:bg-zinc-900 sm:p-5"><b className="block break-words text-xl font-black">{rank.headWaiter.first_name} {rank.headWaiter.last_name}</b><span className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm text-zinc-400"><span>{rank.occupiedTables} / {rank.tables.length} tables occupées</span><span><b className="text-base text-fuchsia-200">{rank.presentPeople}</b> personne{rank.presentPeople !== 1 ? 's' : ''}</span></span><span className="mt-4 flex flex-wrap gap-2" aria-label={`Tables du rang de ${rank.headWaiter.first_name}`}>{rank.tables.map((table) => <RankTableIndicator key={table.id} table={table} />)}</span><span className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-500" aria-label="Légende des tables"><span><span aria-hidden="true" className="text-emerald-400">●</span> Libre</span><span><span aria-hidden="true" className="text-red-400">●</span> Occupée</span></span></button>)}</section>
