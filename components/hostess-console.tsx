...
export function HostessConsole({ tables: initialTables }: { tables: LiveTable[] }) {
  ...
  useEffect(() => {
    async function load() { const { data } = await supabase.from('tables').select('*, zone:zones(*), head_waiter:head_waiters(*), reservation:reservations(*), occupancy:occupancies(*)').eq('active', true).order('display_number'); if (data) setTables(normalise(data)); }
    load();
  ...
  return <>{header}<section className="grid gap-3 sm:grid-cols-2">{waiterTables.map(t=>{const style=tableStyles[computedStatus(t)];return <button onClick={()=>openTable(t)} className="group flex min-h-[104px] items-center rounded-2xl border border-violet-500/30 bg-zinc-900 p-4 text-left shadow-lg shadow-black/20 transition hover:border-violet-400/60 hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-violet-400" key={t.id}><div className="min-w-0"><b className="block text-lg tracking-wide text-white">TABLE {t.display_number}</b><span className="mt-1 block text-sm text-zinc-300">{presentTotal(t)} client{presentTotal(t)!==1?'s':''}</span><span className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold tracking-wide ring-1 ${style.badge}`}>{style.label}</span></div><span className="ml-auto text-xl text-violet-300/70 transition group-hover:translate-x-0.5">›</span></button>})}</section></>;
}
