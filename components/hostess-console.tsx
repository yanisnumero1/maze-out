...
export function HostessConsole({ tables: initialTables }: { tables: LiveTable[] }) {
  ...
  if (screen === 'zones') return <><header className="mb-5"><p className="text-sm uppercase tracking-[.25em] text-fuchsia-400">Arrivées</p><h1 className="text-3xl font-black">ARRIVÉE</h1></header><GlobalSearch tables={tables} drafts={drafts} visits={tableVisits} businessReferrers={businessReferrers} promoters={promoters} onSelectTable={(table) => router.replace(`/hostess?table=${encodeURIComponent(getTableDisplayNumber(table))}`)} onSelectPromoter={(promoter) => router.replace(`/hostess?view=promoteurs&promoter=${encodeURIComponent(promoter.id)}`)} onSelectCdr={(table) => router.replace(`/hostess?zone=${encodeURIComponent(table.zone_id)}`)} /><section className="grid grid-cols-2 gap-3 sm:gap-4">{zones.map((item) => { const summary = stats(tables.filter((table) => table.zone_id === item.id)); return <button className="panel min-h-28 min-w-0 p-4 text-left transition hover:border-violet-400/60 hover:bg-zinc-900 sm:p-5" onClick={() => openZone(item)} key={item.id}><b className="block break-words text-lg sm:text-2xl">{item.name}</b><span className="mt-2 block text-xs leading-5 text-zinc-400 sm:text-sm">{summary.available} table{summary.available !== 1 ? 's' : ''} restante{summary.available !== 1 ? 's' : ''}</span></button>; })}</section></>;
