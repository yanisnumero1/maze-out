...
export function GlobalSearch({ tables, drafts, visits, businessReferrers, promoters, onSelectTable, onSelectPromoter, onSelectCdr }: { tables: LiveTable[]; drafts: ArrivalDraft[]; visits: TableVisit[]; businessReferrers: BusinessReferrer[]; promoters: Promoter[]; onSelectTable: (table: LiveTable) => void; onSelectPromoter: (promoter: Promoter) => void; onSelectCdr: (table: LiveTable) => void }) {
  ...
  return <section ref={rootRef} className="panel mb-5 border border-violet-500/30 bg-zinc-900/70 p-4" aria-label="Recherche globale">
    <label className="block text-sm font-bold text-zinc-200" htmlFor="global-search">Recherche rapide</label>
    <div className="relative mt-2"><input id="global-search" type="search" autoComplete="off" value={input} onFocus={() => setOpen(true)} onChange={(event) => { setInput(event.target.value); setOpen(true); }} onKeyDown={onKeyDown} placeholder="Rechercher une table, réservation, apporteur..." className="min-h-12 w-full rounded-xl border border-zinc-700 bg-zinc-950 py-3 pl-4 pr-12 text-white outline-none placeholder:text-zinc-500 focus:border-violet-400 focus:ring-2 focus:ring-violet-500/30" />{input && <button type="button" aria-label="Effacer la recherche" onClick={() => { setInput(''); setOpen(false); }} className="absolute inset-y-0 right-2 my-auto h-8 rounded-lg px-2 text-zinc-400 hover:bg-zinc-800 hover:text-white">×</button>}</div>
    {open && input.trim() && <div className="mt-3 max-h-[min(30rem,65dvh)] overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-950/90">{visibleResults.length === 0 ? <p className="px-4 py-3 text-sm text-zinc-400">Aucun résultat.</p> : visibleResults.map((result) => {
